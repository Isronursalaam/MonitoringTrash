  ...
  try {
    ...
    for (let attempt = 0; attempt < _KEYS.length; attempt++) {
      ...
      try {
        ...
        // Only rotate on rate limit (429) or auth errors (401/403)
        if (status === 429 || status === 401 || status === 403) {
          continue;
        }
        // For other errors, don't rotate — just fail
        break;
      }
    }

    console.error('[Groq] All keys exhausted:', lastError);
    return res.status(500).json({ error: 'Semua API key habis atau terjadi kesalahan.' });
  } catch (error) {
